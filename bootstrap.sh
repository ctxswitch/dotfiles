...
GOLANG_VERSION=${GOLANG_VERSION:-1.20.1}
...
export PREFIX SCRIPT_PATH GOLANG_VERSION HUGO_VERSION FEX_VERSION KUBERNETES_RELEASE \
  KIND_VERSION IOSEVKA_VERSION GIT_USER_NAME GIT_USER_EMAIL GIT_USER_SIGNINGKEY \
  OS_NAME OS_NAME_LOWER OS_DIST OS_MACHINE OS_ARCH FONT_PATH ADMIN_GROUP HOMEBREW_PATH \
  CARGO_PATH GOLANG_URL KUBECTL_URL KIND_URL IOSEVKA_URL
...
function update-fonts() {
  rm -rf /tmp/iosevka
	...
	rsync -av --delete /tmp/iosevka/* ${SCRIPT_PATH}/fonts
}
...
function configure_all() {
  ...
  # Fonts
  ln -snfF ${SCRIPT_PATH}/fonts ${FONT_PATH}
